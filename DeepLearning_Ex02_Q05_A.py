...
digit_train_images.shape

len(digit_train_labels)

digit_train_labels

digit_test_images.shape

len(digit_test_labels)


#Preprocess the data¶

plt.figure()
plt.imshow(digit_train_images[0])
plt.colorbar()
plt.grid(False)
plt.show()
...
for i in range(25):
    ...
    plt.yticks([])
    plt.grid(False)
    plt.imshow(digit_train_images[i], cmap=plt.cm.binary)
    plt.xlabel(digit_class_names[digit_train_labels[i]])
plt.show()
...
def plot_image(i, predictions_array, true_label, img):
  ...
  plt.xlabel("{} {:2.0f}% ({})".format(fashion_class_names[predicted_label],
                                100*np.max(predictions_array),
                                fashion_class_names[true_label]),
                                color=color)
...
plot_value_array(1, predictions_single[0], fashion_test_labels)
_ = plt.xticks(range(10), fashion_class_names, rotation=45)
